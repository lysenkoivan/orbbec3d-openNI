...
SampleViewer::SampleViewer(const char* strSampleName, openni::Device& device, openni::VideoStream& depth, openni::VideoStream& ir) :
	m_device(device), m_depthStream(depth), m_irStream(ir), m_streams(NULL), m_eViewState(DEFAULT_DISPLAY_MODE), m_pTexMap(NULL)
...
void SampleViewer::onKey(unsigned char key, int /*x*/, int /*y*/)
{
	...
	case '8':
		...
		m_irStream.stop();
		size = 4;
		m_device.getProperty(openni::OBEXTENSION_ID_LDP_EN, (uint8_t*)&ldp_en, &size);
		if (ldp_en == 0x00)
		{
			ldp_en = 0x01;
			m_device.setProperty(openni::OBEXTENSION_ID_LDP_EN, (uint8_t*)&ldp_en, 4);
			printf("turn on ldp...\n");
		}
		else if (ldp_en == 0x01)
		{
